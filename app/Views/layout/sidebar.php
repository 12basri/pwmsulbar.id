<aside
    ...
    <div class="flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <nav class="flex-1 px-4 py-4 space-y-1.5 text-sm font-medium overflow-y-auto custom-scrollbar">
            ...
            <!-- Opini (Menu Baru) -->
            <a href="<?= base_url('admin/opini') ?>" class="flex items-center space-x-3 px-4 py-3 rounded-lg transition <?= url_is('admin/opini*') ? 'bg-white/10 text-white font-semibold' : 'text-slate-200 hover:bg-white/5' ?>">
                <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-data="{ open: <?= url_is('admin/pdm*') ? 'true' : 'false' ?> }">
                ...
                <div x-show="open" x-collapse class="pl-11 pr-4 py-1 space-y-1 text-xs text-slate-300">
                    <a href="<?= base_url('admin/pdm') ?>" class="block py-2 hover:text-white transition <?= url_is('admin/pdm*') ? 'text-yellow-400 font-semibold' : '' ?>">Profil PDM</a>
                </div>
            </div>
